...
if __name__ == "__main__":
    ...
    def upper_case_stream(query: str):
        script = script_retriever(config['embedding_type'], config['embedding_model'], config['db'], query)

        prompt = config['prompt']
        prompt_variables = {"script": script, "question": query, "character": config['character']}
        print(script)
