...
def chat(
    config_yaml: Annotated[
        str, typer.Option(help="config 파일 위치")
    ] = "config_yaml/base.yaml",
    update: Annotated[
        bool, typer.Option("--update", help="서버에서 임베딩 파일 다운로드")
    ] = False,
):
    """챗봇과 대화합니다."""
    ...
    script = script_retriever(
        config["embedding_type"], config["embedding_model"], db, query=user_input
    )
