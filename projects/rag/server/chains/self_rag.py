...
def verification(state: GraphState, llm: BaseLanguageModel) -> GraphState:
    logger.debug("---Answer Check---")
    user_question = state["user_question"]
    ...
    review_system_prompt = """대화의 맥락을 잘 파악하는 의사소통 능력자이다. 생성한 대화가 대화 흐름 상 올바르거나 창의적인지 검토한다."""
...
def decide_to_end(state):
    logger.debug("---ASSESS GENERATION---")
