"""Modified from https://github.com/redotvideo/mamba-chat/blob/main/trainer/data.py"""
...
class ChatDataset(Dataset):
    def __init__(
        self, data_path: str, tokenizer: transformers.AutoTokenizer, conversation_template: str, max_tokens: int
    ):
...
class ChatDataModule:
    def __init__(
        self, tokenizer: transformers.PreTrainedTokenizer, data_path: str, conversation_template, max_tokens: int
    ):

        self.dataset = ChatDataset(
            tokenizer=tokenizer, data_path=data_path, conversation_template=conversation_template, max_tokens=max_tokens
        )
...
def preprocess(
    ...
    for conv in tqdm(conversations):
        ...
        tokenized_conv = tokenizer.apply_chat_template(
            current_conv, chat_template=conversation_template, max_length=max_tokens, truncation=True
        )
